...
public class UnusedItemsMutator implements Alterer<ItemGene, Vec<int[]>> {
	...
	private Phenotype<ItemGene, Vec<int[]>> addUnusedItems( final Phenotype<ItemGene, Vec<int[]>> individual,
		...
		final var genotype =
				Genotype.of( new KnapsackChromosome( new Knapsack( newItems0, MultiObjectiveProblem.MAX_CAPACITY_0 ) ),
	...
	public Set<Item> addUnusedItems( final Set<Item> itemsFromKnapsack0, final Set<Item> itemsFromKnapsack1,
		...
		MultiObjectiveProblem.ITEMS.stream().filter( item -> !itemsFromKnapsack0.contains( item ) ) // Filter items from first knapsack.
